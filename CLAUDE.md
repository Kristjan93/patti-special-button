...
- `ButtFrames/` — Xcode folder reference containing 47 butt icon sets, each in its own subfolder with numbered grayscale 40x40 PNG frames and a `manifest.json`. Generated by `buttsss/brazilian-butt-lift.py`.
- `buttsss/fractured-but-whole/` — Source animated GIFs (512x512).
- `buttsss/brazilian-butt-lift.py` — Python script that extracts, converts to grayscale, and resizes GIF frames into `ButtFrames/`.
